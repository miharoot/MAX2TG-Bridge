...
- **Никаких данных из живых логов в репозитории.** Логи, которые присылает
  ...
  Перед каждым коммитом проверять диф на такие значения — однажды они уже
  просочились в README и тесты, и вычищать пришлось отдельным коммитом
  (`3465429`), а из уже опубликованной истории на GitHub это не убрать
  без перезаписи и обращения в поддержку.
