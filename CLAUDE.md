...
Перед любым `git push` (в любую ветку, включая `main`) — обязательно спросить разрешения у пользователя и дождаться явного подтверждения. Коммитить локально можно свободно, но отправлять на GitHub без подтверждения нельзя.
При пуше баг-фикса добавляем к версии 0.0.1, при пуше новой фичи 0.1.0. Добавляй описание новых фич в Readme и отдельно вкратце в описание отличий этого форка вверху Readme.
Делай Release с версией.
