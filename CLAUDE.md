...
- **Никаких данных из живых логов в репозитории.** Логи, которые присылает
  ...
  Никогда не записывать в гит ссылку на сессию (`Claude-Session:` /
  `https://claude.ai/code/session_…`) — ни в сообщения коммитов, ни в код,
  ни в описания PR и релизов, ни в любые другие файлы репозитория; и
  никогда не делать по ней Share.
...
3. **После получения разрешения, но перед самим пушем** — прогнать `pytest -q`, если менялся код (`app/`, `tests/`); для чисто документационных правок (README.md, CLAUDE.md и т.п. без изменений в коде) тесты не нужны. Повторно гонять тесты не надо, если код не менялся с прошлого прогона: бамп `VERSION`, правки `CHANGELOG.md`/README и любые другие изменения вне `app/` и `tests/` результат прогона не меняют — хватает того, что уже зелёное.
4. **На каждый push, где меняется код `app/`** (не документация) — обязательно, автоматически, без напоминания:
   ...
   - запушить ветку;
   - релиз **не создавать руками**: этим занимается workflow
     `.github/workflows/release-on-version-bump.yml` — он срабатывает на push
     в `main`, меняющий файл `VERSION`, и через `gh release create` сам ставит
     тег `vX.Y.Z` на этот коммит `main` и публикует релиз, собирая заметки из
     сабджектов коммитов со времени прошлого тега (`feat:` → «Новое», `fix:` →
     «Исправления», остальное → «Прочее»). Значит, тег руками не пушить и
     релиз не заводить — достаточно, чтобы коммит с бампом оказался в `main`.
     Пока работа лежит в отдельной ветке, релиза не будет: workflow смотрит
     только на `main`, и релиз выйдет после вливания ветки (обычным мержем или
     squash — в обоих случаях `VERSION` на `main` меняется). Само вливание или
     PR — только по просьбе пользователя.
5. Если push — только документация (без изменений `app/`/`tests/`), пункт 4 пропускается: версию не бампать, `CHANGELOG.md` не трогать, релиз не делать, просто пушить коммит. Правки самого `CLAUDE.md` — ровно этот случай: они меняют инструкции для ассистента, а не поведение моста, поэтому ни записи в `CHANGELOG.md`, ни бампа версии не требуют, даже когда едут в одном пуше с кодом (в разделе `CHANGELOG.md` описывается только то, что изменилось в `app/`).
6. Если несколько локальных коммитов копятся между пушами — версию бампать один раз при итоговом пуше, отражая в разделе `CHANGELOG.md` все вошедшие изменения, а не на каждый отдельный коммит. Текст релиза отдельно не пишется — его собирает workflow из сабджектов коммитов (см. пункт 4), поэтому сабджекты держать внятными и с правильным префиксом (`fix:`/`feat:`/`chore:`): по ним читают релиз.
...
8. Перед пушем показать всё, что будет изменено в гите: список коммитов и
   диф, какая будет версия, какие теги и что будет записано в `CHANGELOG.md`.
