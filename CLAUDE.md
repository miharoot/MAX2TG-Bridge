...
- `/u/<token>` может не открываться через `/add`: pymax понимает только `join/<токен>`, остальное уходит в `LINK_INFO` (опкод 89) и зависит от ответа MAX. Профильные ссылки `max.ru/id<цифры>` обрабатываются локально — id личного чата это XOR двух user_id (`Client.get_chat_id`), см. `PyMaxClient.open_dialog_with_user`.
