...
Покрытие: `app/topics.py` (TopicStore, включая мульти-группу), `app/config.py` (загрузка env, включая `MAX_CHAT_ROUTES`), `app/tg_sender.py` (резолвинг целевой группы, `ensure_topic`), `app/max_listener.py` (форматирование, throttle), `app/tg_handler.py` (роутинг команд и медиа), `app/max_client.py` (опкоды). 214 тестов.
