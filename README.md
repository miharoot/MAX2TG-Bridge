...
Команды работают в **любой** супергруппе, куда добавлен бот (не только в `TG_CHAT_ID`) — `/bind` и `/add` создают топик именно в той группе, где их вызвали, тем самым направляя туда этот MAX-чат. При `TG_ALLOWED_USER_ID` — только от этого пользователя.
