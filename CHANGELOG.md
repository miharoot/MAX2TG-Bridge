...
- Текущая стабильная.
- новая команда `/read` внутри топика: отмечает чат прочитанным в MAX,
  не отвечая в нём. Раньше отметка уходила только вместе с ответом —
  Telegram не сообщает боту, что переписку прочитали, такого события у
  ботов нет, — и собеседник видел сообщение непрочитанным, пока вы не
  ответите. Если мост не знает ни одного сообщения этого чата или MAX
  отметку отклонил, команда говорит об этом, а не молчит
- в `/help` описано, что означают реакции, которые бот ставит в топике:
  🕊 — MAX принял ваше сообщение, 👀 — собеседник прочитал переписку
- README: `/read` добавлена в список команд и в таблицу, а строка про
  проброс отметок прочтения приведена в соответствие с тем, какие
  реакции мост ставит на самом деле

## 0.8.4 — 2026-09-18

- отметка прочтения не появлялась в топике никогда: Telegram принимает от
  бота только свой фиксированный набор реакций, и того эмодзи, что стоял
  в коде, в нём нет — библиотека отправляла его как id кастомного эмодзи,
  а API отвечал отказом. Отказ писался в лог на уровне debug и потому был
  не виден
- теперь прочтение собеседника отмечается 👀, а подтверждение того, что
  MAX принял ваше сообщение, — 🕊; оба эмодзи из разрешённого набора
- реакция с эмодзи вне набора вообще не отправляется, а в лог идёт прямое
  объяснение — чтобы эта ошибка не вернулась молча
- отказ Telegram по существу (сообщение удалено, слишком старое, реакции
  в чате запрещены) больше не повторяется тремя попытками: повтор его не
  лечит, а лог засоряет
